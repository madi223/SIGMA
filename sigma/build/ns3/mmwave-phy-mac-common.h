...
public:
  ...
  void ComputeAll();
  void SetDlSymb(uint8_t m_ulSymb);
  void SetUlSymb(uint8_t m_ulSymb);
  
  void SetUlMcs(uint8_t m_ulmcs)
  {
    this->m_ulmcs=m_ulmcs;
  }

   void SetDlMcs(uint8_t m_dlmcs)
  {
    this->m_dlmcs=m_dlmcs;
  }
...
public:
  ...
  // madi
        ...
        { 
          uint8_t allsum = 0;
          for(auto it = m_prbTable.begin(); it != m_prbTable.end(); it++)
            {
              m_prbTable[it->first].ComputeAll();
              allsum +=  m_prbTable[it->first].GetAllSymb();
            }
          return allsum;
        }
  
        void
        AddUeInPrbUlTab(uint16_t rnti, UePrb myUE)
        {
          if (m_prbTable.find(rnti) == m_prbTable.end())
            m_prbTable[rnti] = myUE;
          else
            m_prbTable[rnti].SetUlSymb(myUE.GetUlSymb());
        }
  
        void
        AddUeInPrbDlTab(uint16_t rnti, UePrb myUE)
        {
          if (m_prbTable.find(rnti) == m_prbTable.end())
            m_prbTable[rnti] = myUE;
          else
            m_prbTable[rnti].SetDlSymb(myUE.GetDlSymb());
        }

       inline UePrb
       GetUeFromRnti(uint16_t rnti)
       {
         if (m_prbTable.find(rnti) != m_prbTable.end())
           return m_prbTable[rnti];
         UePrb empty = UePrb();
         empty.SetState(false);
         return empty;
       }
...
private:
  ...
  // madi Resource allocation table
   std::map<uint16_t,UePrb>  m_prbTable;
};

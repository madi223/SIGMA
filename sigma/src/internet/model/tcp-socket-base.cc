...
TcpSocketState::TcpSocketState (const TcpSocketState &other)
  : Object (other),
    ...
    m_ranBdp(other.m_ranBdp),
    m_ranBitRate(other.m_ranBitRate),
    m_ranTimer(other.m_ranTimer),
    m_ranNbRtts(other.m_ranNbRtts),
    m_ranRttTimer(other.m_ranRttTimer),
    m_isRanFull(other.m_isRanFull),
    m_prevCwnd(other.m_prevCwnd),
...
void
TcpSocketBase::SetRanBdp (uint32_t bdp)
{ // madi
  m_tcb->m_ranBitRate = bdp;
  if (!m_tcb->m_isRanFull){
    m_tcb->m_pacing = true;
    m_tcb->m_maxPacingRate = DataRate(std::to_string((m_tcb->m_ranBitRate*8)/1000)+"Kb/s");
    m_tcb->m_currentPacingRate = DataRate(std::to_string((m_tcb->m_ranBitRate*8)/1000)+"Kb/s");
    std::cout<<"[EdgeCC] : PACING IS ACTIVE [SS]"<<std::endl;
  }
  ...
  std::cout<<"Utilization from TCP["<<ranUsage<<"]"<<std::endl;
  GlobalValue::GetValueByName ("RttMs", doubleValue);
  double RTTst = doubleValue.Get ();
  double myRTT = ((Time)(m_tcb->m_minRtt)).GetSeconds () > 1000 ? RTTst/1000 : ((Time)(m_tcb->m_minRtt)).GetSeconds ();
  double lastRtt = ((Time)(m_tcb->m_lastRtt)).GetSeconds ();
  double localUtilz = 0;
  localUtilz = m_tcb->m_ranBdp !=0  ? ((double)((uint32_t)(m_tcb->m_bytesInFlight)))/((double)m_tcb->m_ranBdp):0;
  
  m_tcb->m_prevCwnd = std::max(m_tcb->m_prevCwnd,m_tcb->m_initialCWnd*m_tcb->m_segmentSize);
  if ((lastRtt >= 1.2*myRTT)&&(!m_tcb->m_isRanFull)&&(m_tcb->m_cWnd.Get() >=(/*1.01*/2*m_tcb->m_prevCwnd)/*(1.5*m_tcb->m_initialCWnd*m_tcb->m_segmentSize)*//*localUtilz>=0.8*/)/*m_tcb->m_cWnd.Get() >= m_tcb->m_ranBdp)*/){// Link capacity is exceeded + Pipe was not full before
    m_tcb->m_isRanFull = true;
    m_tcb->m_ranBdp =(bdp*myRTT)*(myRTT/lastRtt); // decrease Bytes inflight
    m_tcb->m_ranBdp = std::min(bdp*myRTT,(double)m_tcb->m_ranBdp);
    m_tcb->m_prevCwnd = m_tcb->m_cWnd.Get();
    // This BDP value is maintained until lastRTT is less than 1.5*minRTT  
  }
  else if ((lastRtt < 1.2*myRTT)&&(m_tcb->m_cWnd.Get() >= (/*1.01*/2*m_tcb->m_prevCwnd)/*(1.5*m_tcb->m_initialCWnd*m_tcb->m_segmentSize)*/)/*m_tcb->m_cWnd.Get () >= m_tcb->m_ranBdp*//*localUtilz >=0.8*//*&&(m_tcb->m_isRanFull)*/){ // RTT becomes close to minRTT thanks to the maintained BDP values
    // Now we have to check whether the link is used optimally
    m_tcb->m_isRanFull = false;
    m_tcb->m_prevCwnd =	m_tcb->m_cWnd.Get();
    if ((ranUsage!=0)&&(ranUsage<0.99)&&(m_tcb->m_ranBdp>14600)){
      uint32_t newBdp = m_tcb->m_ranBdp;
      newBdp += ((1-ranUsage))*((double)m_tcb->m_ranBdp);
      m_tcb->m_ranBdp = std::min((double)newBdp,(double)(bdp)*1.2);//+= ((1-ranUsage))*((double)m_tcb->m_ranBdp);
    }
    else if ((ranUsage==0)||(m_tcb->m_ranBdp<14600))
      m_tcb->m_ranBdp = (bdp*myRTT) >= 14600 ? (bdp*myRTT) : 14600 ;
    else
      m_tcb->m_ranBdp=m_tcb->m_ranBdp;    
  }
  else if(m_tcb->m_cWnd.Get() <= (m_tcb->m_initialCWnd*m_tcb->m_segmentSize)){
       m_tcb->m_ranBdp = (bdp*myRTT) >= 14600 ? (bdp*myRTT) : 14600 ;
       m_tcb->m_prevCwnd = m_tcb->m_initialCWnd*m_tcb->m_segmentSize; 
    }
  //m_tcb->m_ranBdp =(bdp*myRTT)*(myRTT/lastRtt);
  //m_tcb->m_ranBdp = m_tcb->m_ranBdp >= 14600 ? m_tcb->m_ranBdp : 14600;
  std::cout<<"RAN BDP = "<<m_tcb->m_ranBdp<<" Isfull ="<<m_tcb->m_isRanFull<<" Local-UTZ = {"<<localUtilz<<"} minRTT = "<<((Time)(m_tcb->m_minRtt)).GetSeconds () <<"ICWD =["<<m_tcb->m_initialCWnd<<"] SSthresh = {"<<m_tcb->m_initialSsThresh<<"} "<<std::endl;
  //localUtilz = ((double)((uint32_t)(m_tcb->m_bytesInFlight)))/((double)m_tcb->m_ranBdp);
  #endif
}

uint32_t
TcpSocketBase::GetRanBdp(void) const
{
  return m_tcb->m_ranBdp;
}
...
tt)).GetSeconds()*1000000) << "," <<(double(0)*8)/(0.000125*1000000) << "," << 0 << "," <<(uint32_t)(m_tcb->m_bytesInFlight) <<","<< m_tcb->m_congState <<","<< m_endPoint->\
   GetPeerPort () <<","<<m_endPoint->GetLocalPort() <<","<<m_rWnd<<","<<ranUsage<< std::endl;
 // end MELD log 
  return sz;
}
...
{ //if(m_tcb->m_ranBitRate<=0) 
  ...
  double lastRtt = ((Time)(m_tcb->m_lastRtt)).GetSeconds ();
  double localUtilz = 0;
  uint32_t bdp0 = m_tcb->m_ranBdp > 0 ? m_tcb->m_ranBdp : m_tcb->m_ranBitRate*myRTT;
  localUtilz = bdp0 !=0  ? ((double)BytesInFlight() )/((double)bdp0):0;

  m_tcb->m_prevCwnd = std::max(m_tcb->m_prevCwnd,m_tcb->m_initialCWnd*m_tcb->m_segmentSize);
  if ((lastRtt >= 1.5*myRTT)/*&&(m_tcb->m_isRanFull)*//*&&(m_tcb->m_cWnd.Get() > m_tcb->m_initialCWnd*m_tcb->m_segmentSize)*/){// Link capacity is exceeded + Pipe was not full before
    m_tcb->m_pacing = false;
    //m_tcb->m_isRanFull = true;
    //m_tcb->m_ranNbRtts = std::ceil(lastRTT/myRTT);

    //if((m_tcb->m_ranTimer == 0)||(TcpOptionTS::ElapsedTimeFromTsValue(m_tcb->m_ranTimer).GetSeconds()>=m_tcb->m_ranNbRtts*myRTT)){
    uint32_t prevWindow = m_tcb->m_ranBdp;
    int prevNrtts = m_tcb->m_ranNbRtts;
    m_tcb->m_ranBdp =(m_tcb->m_ranBdp/*m_tcb->m_ranBitRate*myRTT*/)*(myRTT/lastRtt); // decrease Bytes inflight
    m_tcb->m_ranNbRtts = std::ceil(lastRtt/myRTT);
    m_tcb->m_ranTimer = TcpOptionTS::NowToTsValue ();
    m_tcb->m_ranBdp = /*std::max(*/std::min(m_tcb->m_ranBitRate*myRTT,(double)m_tcb->m_ranBdp);//,(double)(1*m_tcb->m_segmentSize));
    
    /*if((prevWindow <= 0.01*(m_tcb->m_ranBitRate*myRTT))){ // the bdp has been decreased up to 10MSS
      m_tcb->m_ranBdp = m_tcb->m_ranBitRate*myRTT;
      m_tcb->m_ranNbRtts = 1;
      m_tcb->m_isRanFull = false;
    }*/
    m_tcb->m_prevCwnd = m_tcb->m_cWnd.Get();
  ...
  else if ((lastRtt < 1.5*myRTT) && (m_tcb->m_cWnd.Get() > m_tcb->m_initialCWnd*m_tcb->m_segmentSize)){ // RTT becomes close to minRTT thanks to the maintained BDP values                  
    // Now we have to check whether the link is used optimally
    m_tcb->m_pacing = false;
    m_tcb->m_isRanFull = false;
    m_tcb->m_prevCwnd = m_tcb->m_cWnd.Get();
    if ((ranUsage!=0)&&(ranUsage<0.95)&&(m_tcb->m_ranBdp>14600)){ //0.99
      uint32_t newBdp = m_tcb->m_ranBdp;
      newBdp += ((0.96-ranUsage))*((double)m_tcb->m_ranBdp);  //1
      //if((m_tcb->m_ranRttTimer == 0)||(TcpOptionTS::ElapsedTimeFromTsValue(m_tcb->m_ranRttTimer).GetSeconds()>=myRTT)){
      m_tcb->m_ranBdp = std::min((double)newBdp,(double)(m_tcb->m_ranBitRate*myRTT)*1.5/*1.2*/);//+= ((1-ranUsage))*((double)m_tcb->m_ranBdp);
      m_tcb->m_ranRttTimer = TcpOptionTS::NowToTsValue ();
      //}
    }
    else if ((ranUsage==0)||(m_tcb->m_ranBdp<14600))
      m_tcb->m_ranBdp = (m_tcb->m_ranBitRate*myRTT) >= 14600 ? (m_tcb->m_ranBitRate*myRTT) : 14600 ;
    else
      m_tcb->m_ranBdp=m_tcb->m_ranBdp;
  }
  else if(m_tcb->m_cWnd.Get() <= (m_tcb->m_initialCWnd*m_tcb->m_segmentSize)){
       m_tcb->m_ranBdp = (m_tcb->m_ranBitRate*myRTT) >= 14600 ? (m_tcb->m_ranBitRate*myRTT) : 14600 ;
       m_tcb->m_prevCwnd = m_tcb->m_initialCWnd*m_tcb->m_segmentSize;
       std::cout<<"INITIAL STEP ONCE[...]"<<std::endl;
       //m_tcb->m_pacing = true;
       //m_tcb->m_maxPacingRate = /*DataRateValue(*/DataRate(std::to_string((m_tcb->m_ranBdp*8)/1000)+"Kb/s");//);
       //m_tcb->m_currentPacingRate = DataRate(std::to_string((m_tcb->m_ranBdp*8)/1000)+"Kb/s"); 
       m_tcb->m_pacing = true;                                                                                                                                      m_tcb->m_maxPacingRate = /*DataRateValue(*/DataRate(std::to_string((m_tcb->m_ranBitRate*8)/1000)+"Kb/s");//);
       m_tcb->m_currentPacingRate = DataRate(std::to_string((m_tcb->m_ranBitRate*8)/1000)+"Kb/s"); 
    }

std::cout<<"RAN BDP = "<<m_tcb->m_ranBdp<<" Isfull ="<<m_tcb->m_isRanFull<<" Local-UTZ = {"<<localUtilz<<"} minRTT = "<<((Time)(m_tcb->m_minRtt)).GetSecond\
s () <<"ICWD =["<<m_tcb->m_initialCWnd<<"] SSthresh = {"<<m_tcb->m_initialSsThresh<<"} "<<std::endl;
 m_tcb->m_cWnd = m_tcb->m_initialCWnd*m_tcb->m_segmentSize*50;//m_tcb->m_ranBdp; //  m_tcb->m_initialCWnd*m_tcb->m_segmentSize*50;//m_tcb->m_ranBdp;  
 return std::min (m_tcb->m_ranBdp/*m_rWnd.Get ()*/,m_tcb->m_cWnd.Get()/*m_rWnd.Get ()*//*m_tcb->m_initialCWnd*m_tcb->m_segmentSize*50*/ /*m_tcb->m_cWnd.Get ()*/);
 #endif
}

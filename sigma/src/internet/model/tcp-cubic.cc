          ...
          if ((now - m_roundStart) > m_delayMin)
            {
              ...
              std::cout<<now<<": PKT train exit : "<<" DelayMin = "<<m_delayMin<<" Current = "<<now - m_roundStart<<std::endl;
            }
        }
      ...
      if (m_sampleCnt < m_hystartMinSamples)
      ...
      else
        {
          if (m_currRtt > m_delayMin +
            ...
            {
              m_found |= DELAY;
              std::cout<<now<<": RTT increase exit : "<<" RTT = "<<m_currRtt<<" DelayMin = "<<m_delayMin<<std::endl;
            }
        }
      /*
      ...
      if (m_found & m_hystartDetect)
        {
          ...
          std::cout<< now<<": Exit from SS, immediately :-)"<<std::endl;
        }
    }
}

...
MmWaveUeMac::DoReceiveControlMessage  (Ptr<MmWaveControlMessage> msg)
{
	...
	case (MmWaveControlMessage::DCI_TDMA):
		...
		DciInfoElementTdma dciInfoElem = dciMsg->GetDciInfoElement ();
                ...
                /****************** MADI PRB TABLE UPDATE *****************/
                if (dciInfoElem.m_format == DciInfoElementTdma::UL_dci){
                  Ptr<MmWavePhyMacCommon> mymac = this->GetConfigurationParameters();
                    //uint32_t maxSym = mymac->GetSymbPerSlot()-1;
                    //Ptr<MmWaveAmc> myamc = CreateObject <MmWaveAmc> (mymac);
                    //uint32_t tbsmax = (myamc->GetTbSizeFromMcsSymbols(dciInfoElem.m_mcs,maxSym) /8);
                    UePrb myUE = UePrb((unsigned)dciInfoElem.m_numSym);
                    myUE.SetUlMcs(dciInfoElem.m_mcs);
                ...
                if (dciInfoElem.m_format == DciInfoElementTdma::DL_dci){
                  ...
                  //uint32_t tbsmax = (myamc->GetTbSizeFromMcsSymbols(dciInfoElem.m_mcs,maxSym) /8);
                  UePrb myUE = UePrb(); //((unsigned)dciInfoElem.m_numSym);
                  myUE.SetDlSymb((unsigned)dciInfoElem.m_numSym);
                  myUE.SetDlMcs(dciInfoElem.m_mcs);
                  mymac->AddUeInPrbDlTab(dciInfoElem.m_rnti,myUE);
                  uint8_t allocated = mymac->GetPrbTabSum();
                  
                }
                /****************** END MADI PRB TAB UPDATE **************/

		if (dciInfoElem.m_format == DciInfoElementTdma::UL_dci)
                  {//madi
                    //std::cout<<"RNTI = "<<dciInfoElem.m_rnti<<" UL_TBS = "<<dciInfoElem.m_tbSize<<std::endl;
                    /*Ptr<MmWavePhyMacCommon> mymac = this->GetConfigurationParameters();
                    uint32_t maxSym = mymac->GetSymbPerSlot();
                    Ptr<MmWaveAmc> myamc = CreateObject <MmWaveAmc> (mymac);
                    uint32_t tbsmax = (myamc->GetTbSizeFromMcsSymbols(dciInfoElem.m_mcs,maxSym) /8);
                    UePrb myUE = UePrb((unsigned)dciInfoElem.m_numSym);
                    mymac->AddUeInPrbUlTab(dciInfoElem.m_rnti,myUE);
                    uint8_t allocatedUL = mymac->GetPrbTabSum();
                    std::cout<<"MaxSymb = "<<maxSym<<" RNTI = "<<dciInfoElem.m_rnti<<" UL_TBS = "<<dciInfoElem.m_tbSize<<" MaxTBS = "<<tbsmax<<" AllocatedUL ="<<(unsigned)allocatedUL<<std::endl;*/

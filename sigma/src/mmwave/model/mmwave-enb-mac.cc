		...
		if (slotAllocInfo.m_slotType != SlotAllocInfo::CTRL && slotAllocInfo.m_tddMode == SlotAllocInfo::DL_slotAllocInfo)
		{
			...
			else
			{
				...
				uint8_t tbUid = dciElem.m_harqProcess;
                                #if 0
                                /************** Madi Update DL RB *******************/
                                if (dciElem.m_format == DciInfoElementTdma::DL_dci){
                                  Ptr<MmWavePhyMacCommon> mymac = this->GetConfigurationParameters();
                                  UePrb myUE = UePrb(); //((unsigned)dciInfoElem.m_numSym);
                                  std::cout<<"[RNTI = "<<dciElem.m_rnti<<"] DL Sym Found !! Sym = "<<(unsigned)dciElem.m_numSym<<std::endl;
                                  myUE.SetDlSymb((unsigned)dciElem.m_numSym);
                                  myUE.SetDlMcs(dciElem.m_mcs);
                                  mymac->AddUeInPrbDlTab(dciElem.m_rnti,myUE);
                                }
                                /************* END MADI Update *********************/
                                #endif
